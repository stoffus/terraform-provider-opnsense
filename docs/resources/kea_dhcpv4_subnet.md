---
page_title: "opnsense_kea_dhcpv4_subnet Resource - terraform-provider-opnsense"
subcategory: Kea
description: |-
  Configure DHCPv4 subnets for Kea.
---

# opnsense_kea_dhcpv4_subnet (Resource)
...
# Full resource
resource "opnsense_kea_dhcpv4_subnet" "example" {
  subnet = "10.8.0.0/16"
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import opnsense_kea_dhcpv4_subnet using the `id`. For example:

```terraform
import {
  to = opnsense_kea_dhcpv4_subnet.example
  id = "<opnsense-resource-id>"
}
```

Using `terraform import`, import opnsense_kea_dhcpv4_subnet using the `id`. For example:

```console
% terraform import opnsense_kea_dhcpv4_subnet.example <opnsense-resource-id>
```
